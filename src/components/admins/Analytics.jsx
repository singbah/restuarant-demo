...
export default function Analytics() {
  ...
  const { data, loading, refetch } = useFetch(`/admin/analytics`);
  ...
  async function deleteContact(contactId, type) {
    ...
    type === "contact"
      ? (url = `/admin/contact?contact_id=${contactId.id}`)
      : (url = `/admin/unsubscribe?subscriber_id=${contactId.id}`);
    try {
      ...
      setMsg({
        ...
        title: "error",
        status: "success",
        msg: error.response?.data.detail || "An error Occur while  deleting",
      });
    }
  }
  ...
  async function publishPost(post_id) {
    try {
      await api.patch(`/admin/publish_post?post_id=${post_id}`);
      refetch();
    } catch (error) {
      console.log(error);
    }
    ...
    <Dashboard
      component={
        <div className="w-full overflow-y-auto">
          {admin ? (
            <section className="p-2 w-full overflow-x-hidden">
              {/* HEADER HERE */}
              {!loading ? (
                <>
                  <AlertCard
                  ...
                  <section className="flex gap-4 justify-center items-center">
                    <KyiCard
                      title={"Visitors"}
                      color={"blue"}
                      value={viewFormater(data && data.visitor_count)}
                      percentage={10}
                      icon={
                        <BsPeople
                          className="text-blue-700 bg-blue-200 p-1 rounded-xl"
                          size={50}
                        />
                      }
                    />
                    ...
                    <KyiCard
                      title={"Posts Views"}
                      color={"green"}
                      value={viewFormater(data && data.views)}
                      percentage={+10}
                      icon={
                        <ViewIcon
                          className="text-green-700 bg-green-200 p-1 rounded-xl"
                          size={50}
                        />
                      }
                    />
                    ...
                    <KyiCard
                      ...
                      icon={
                        <GiPhotoCamera
                          className="text-purple-700 bg-purple-200 p-1 rounded-xl"
                          size={50}
                        />
                      }
                    />
                    ...
                    <KyiCard
                      ...
                      color={"yellow"}
                      value={data ? data.contacts_count : 0}
                      percentage={5}
                      icon={
                        <MessageCircle
                          className="text-yellow-700 bg-yellow-200 p-1 rounded-xl"
                          size={50}
                        />
                      }
                    />
                    ...
                    <KyiCard
                      title={"Comments"}
                      color={"green"}
                      value={data ? data.comments_count : 0}
                      percentage={5}
                      icon={
                        <MessageCircle
                          className="text-green-700 bg-green-200 p-1 rounded-xl"
                    ...
                    <KyiCard
                      title={"Subscribers"}
                      color={"orange"}
                      value={data ? data.newsletters_count : 0}
                      percentage={5}
                      icon={
                        <FcNews
                          className="text-orange-700 bg-orange-200 p-1 rounded-xl"
                          size={50}
                        />
                      }
                    />
                    ...
                    <div className="overflow-y-auto border-2 bg-green-100 border-gray-200 rounded-xl shadow shadow-black p-2 m-2 h-96">
                      ...
                      {data &&
                        data.comments.map((comment) => (
                          <div className="p-2 m-2 border-2 rounded-xl shadow bg-white shadow-black relative overflow-y-auto hover:scale-102 transition cursor-pointer">
                  ...
                  <section className="grid grid-cols-3 items-start">
                      ...
                      {data && data.posts.length > 0 ? (
                        data.posts.map((blog, index) => (
                          ...
                          >
                            ...
                            <p className="text-gray-400 text-[10px] italic p-1">
                            ...
                            {blog.status === "true" ? (
                              <p
                                ...
                                className="text-sm absolute top-2 right-2 text-green-700 bg-green-200 p-1 rounded-xl"
                              >
                                {blog.status}
                              </p>
                            ) : (
                              <p
                                onClick={() => publishPost(blog.id)}
                                className="text-sm absolute top-2 right-2 text-red-700 bg-red-200 p-1 rounded-xl"
                              >
                                {blog.status}
                              </p>
                            )}
                        ...
                        <button
                          onClick={() => navigate("/admin/post/editor")}
                          className="m-4 p-4 bg-green-600 rounded-2xl font-black text-white text-2xl cursor-pointer hover:bg-green-800 active:scale-110 transition"
                        >
                          {" "}
                          Add New Post <PlusIcon className="inline" size={35} />
                        </button>
                      )}
                    </div>
                      ...
                      {data &&
                        data.contacts.map((contact, index) => (
                          ...
                          >
                            ...
                            <p className="text-sm absolute top-2 right-2 text-yellow-700 bg-yellow-200 p-1 rounded-xl">
                              {contact.status}
                            </p>
                            <p className="text-gray-400 text-[10px] italic p-1">
                              {formatDistanceToNow(
                            ...
                            <button
                              onClick={() =>
                                setMsg({
                                  ...
                                  msg: "You will delete this contact",
                                  open: true,
                                  status: "error",
                                  action: () =>
                              ...
                              className="px-4 bg-red-600 rounded-lg font-bold text-white absolute right-2 bottom-2 shadow shadow-black active:scale-105 active:bg-red-800 transition"
                            >
                              {" "}
                              Delete{" "}
                            </button>
                    ...
                    <div className="overflow-y-auto border-2 bg-orange-100 border-gray-200 rounded-xl shadow shadow-black p-2 m-2 h-96">
                      ...
                      {data &&
                        data.newsletter.map((subscriber, index) => (
                          <div
                            key={index}
                            className="p-2 m-2 border-2 rounded-xl bg-white shadow shadow-black relative overflow-y-auto hover:scale-102 transition cursor-pointer"
                          >
                            ...
                            <button
                              onClick={() =>
                                setMsg({
                                  ...
                                  open: true,
                                  status: "error",
                                  action: () => deleteContact(subscriber),
                                })
                              }
                              className="px-4 bg-red-600 rounded-lg font-bold text-white absolute right-2 bottom-2 shadow shadow-black active:scale-105 active:bg-red-800 transition"
                            >
