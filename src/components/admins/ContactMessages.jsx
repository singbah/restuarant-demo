...
export default function ContactMessage() {
  ...
  async function getMessages() {
    ...
    try {
      const response = await api.get(
        `/admin/all-contacts?cursor=${Number(cursor) || 0}&limit=${Number(hasMore)}`,
      );
      ...
      console.error(
        ...
        error.response?.data.detail || "An error Occur",
      );
      setMsg({
        title: "Error",
  ...
  async function getNewsLetter() {
    ...
    try {
      const response = await api.get(
        `/admin/all-newsletters?cursor=${Number(lastLid) || 0}&limit=${Number(limit)}`,
      );
      const data = response.data;
      ...
      component={
        <div className="m-4 w-full flex flex-row">
          ...
          <div className="border w-full overflow-y-auto rounded-2xl">
            <h1 className="p-4 mx-4 text-3xl text-center font-bold">
              Conversation
            </h1>
            {activeTab === null && (
              <div>
                <h1 className="text-2xl font-bold m-2 p-4">
                  Username : {mailTo.name}
                </h1>
                <p className="px-4 font-semibold italic ">
                  User Email : {mailTo.email}
                </p>
          ...
          <div className="overflow-y-auto border-2 border-gray-200 rounded-xl shadow shadow-black p-2 m-2 w-1/2 relative">
            <article className="flex justify-center items-center p-4 gap-4 text-lg border-b-2 border-gray-500">
              <button
                ...
                className="focus:border font-semibold focus:font-extrabold px-2 rounded focus:bg-green-300 transition text-gray-500 focus:scale-105"
              >
                Subscribers
              </button>
              <button
            ...
            {switchUser === null &&
              newletters.map((subscriber, index) => (
                <div
                  key={index}
                  onClick={() => {
                    setMailTo((prev) => ({
                ...
                >
                  ...
                  <p className="text-sm absolute top-2 right-2 text-green-700 bg-green-200 p-1 rounded-xl">
                    {subscriber.status}
                  </p>
                  <p className="text-gray-400 text-[10px] italic p-1">
                    {Fdate(subscriber.created_at)}
                  </p>
                  <p className="text-gray-400 text-[10px] italic p-1">
                    {Fdate(subscriber.last_open)}
                  </p>
                </div>
              ))}
